...
import { Check, X, Plus, BarChart3, Trophy, Target, TrendingUp, TrendingDown, Zap, Star, Brain, Crown } from 'lucide-react';
...
const QuizScoresheet = () => {
  ...
  const updateAnswer = (roundIndex, questionIndex, answer) => {
    const newRounds = [...rounds];
    newRounds[roundIndex].answers[questionIndex] = answer;
    setRounds(newRounds);
  };

  const updateSecondAnswer = (roundIndex, questionIndex, answer) => {
    const newRounds = [...rounds];
    newRounds[roundIndex].secondAnswers[questionIndex] = answer;
    setRounds(newRounds);
  };

  const updateTopFiveAnswer = (roundIndex, questionIndex, answerIndex, answer) => {
    const newRounds = [...rounds];
    newRounds[roundIndex].topFiveAnswers[questionIndex][answerIndex] = answer;
    setRounds(newRounds);
  };

  const markQuestion = (roundIndex, questionIndex, isCorrect) => {
    const newRounds = [...rounds];
  ...
  const markSecondQuestion = (roundIndex, questionIndex, isCorrect) => {
    const newRounds = [...rounds];
    newRounds[roundIndex].secondCorrect[questionIndex] = isCorrect;
    setRounds(newRounds);
  };

  const markTopFiveAnswer = (roundIndex, questionIndex, answerIndex, isCorrect) => {
    const newRounds = [...rounds];
            ...
            <div className="bg-white/10 backdrop-blur-lg rounded-2xl p-4 border border-white/20">
              <h2 className="text-xl font-bold mb-4 text-center text-white">
                ...
                <span className="block text-sm text-white/80 mt-1">
                  {round.questions} questions • {roundFormats[round.format]?.description}
                </span>
              </h2>
              ...
              <div className="mb-4">
                ...
                <p className="text-xs text-white/60 mt-1 text-center">
                  {roundFormats[round.format]?.description}
                </p>
              </div>
                ...
                <div className="space-y-3">
                  {Array.from({ length: round.questions }, (_, questionIndex) => (
                    <div key={questionIndex} className="bg-white/5 p-3 rounded-xl border border-white/10">
                      ...
                      {/* Second answer input for Easy/Hard and All-or-Nothing formats */}
                      ...
                      <div className="flex space-x-2 justify-center">
                        ...
                        {/* Second answer buttons for Easy/Hard and All-or-Nothing */}
                        {(round.questionFormat === 'easyhard' || round.questionFormat === 'allornothing') && (
                          <div className="flex space-x-1 flex-1">
                            <button
                      ...
                      {/* Score indicator for this question */}
